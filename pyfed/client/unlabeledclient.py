...
class UnlabeledClient(object):
    ...
    def _setup(self):
        ...
        self.train_unlabeled_loader, self.valid_loader, self.test_loader = build_dataset(
    ...
    def train(self, server_model=None):
        ...
        for _ in range(len(self.train_unlabeled_loader)):
            image_weak, image_strong, label= next(iter(self.train_unlabeled_loader))
            image_weak, image_strong, label = image_weak.to(self.device), image_strong.to(self.device), label.to(self.device)
            with torch.no_grad():
                fw, output = self.model(image_weak)
        ...
        loss_1 = loss_all / len(self.train_unlabeled_loader)
        self.total_dis_1 /= len(self.train_unlabeled_loader)
        self.round += 1
        self.model.to('cpu')
        return loss_1, acc
    ...
    def client_to_server(self):
        return {'model': self.model,
                ...
                'data_len': len(self.train_unlabeled_loader)}

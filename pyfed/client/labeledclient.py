...
class LabeledClient(object):
    ...
    def _setup(self):
        ...
        self.train_labeled_loader, self.valid_loader, self.test_loader = build_dataset(self.config, self.site)
    ...
    def train(self, server_model=None):
        ...
        # for step, (image, label) in enumerate(self.train_labeled_loader):
        for _ in range(len(self.train_labeled_loader)):
            ...
            if self.iter_round <= 0:
                ...
                local = torch.nn.Softmax(dim=1)(self.dis_1 / 0.01)
                last = torch.nn.Softmax(dim=1)(self.other_dis_1.detach() / 0.01)
                score_1 = surprise_soft_bins(local, last)
                loss_dis = 1 * score_1.mean()
                loss = self.loss_fn(output, label) + loss_dis
            loss_all += loss.item()

...
class FedProxClient(BaseClient):
    def __init__(self, config, site, server_model):
        super(FedProxClient, self).__init__(config, site, server_model)
    ...
    def train(self, server_model=None):
        ...
        for step in range(len(self.train_labeled_loader)):
            ...
            # FIXME
            # train_acc += DiceLoss().dice_coef(output, label).item()
        ...
        # acc = train_acc / len(self.train_loader)
        self.round += 1
